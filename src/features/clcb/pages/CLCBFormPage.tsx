...
export const CLCBFormPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadCLCB = async () => {
      setLoading(true);
      try {
        const data = await clcbService.getById(id) as any;
        form.setFieldsValue({
          ...data,
          dataContrato: data?.dataContrato ? dayjs(data.dataContrato) : dayjs(),
        });
      } catch (error: any) {
        message.error('Erro ao carregar CLCB: ' + error.message);
        navigate('/clcb');
      } finally {
        setLoading(false);
      }
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      if (isEditing && id) {
        await clcbService.update(id, formattedValues);
      ...
      >
        <Space orientation="vertical" size={0}>
          <Title level={isMobile ? 3 : 2} style={{ margin: 0 }}>
            {isEditing ? 'Editar CLCB' : 'Cadastrar Novo CLCB'}
          </Title>
          <Text type="secondary">Gerencie Certificados de Licença do Corpo de Bombeiros.</Text>
        </Space>
        <Button icon={<ArrowLeftOutlined />} onClick={() => navigate('/clcb')} style={{ width: isMobile ? '100%' : 'auto' }}>
          Voltar
        </Button>
      </div>
          ...
          <Row gutter={[24, 0]}>
            ...
            <Col xs={24} lg={10}>
              ...
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
                <Button style={{ background: isDarkMode ? '#171C2A' : '#fff' }} onClick={() => navigate('/clcb')}>
                  Cancelar
                </Button>
                <Button type="primary" icon={<SaveOutlined />} htmlType="submit" loading={saving}>
                  {isEditing ? 'Salvar Alterações' : 'Cadastrar CLCB'}
                </Button>
              </div>
            </Col>
          </Row>
