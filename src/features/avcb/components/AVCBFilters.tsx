...
export const AVCBFilters: React.FC<AVCBFiltersProps> = ({ onSearch, onClear }) => {
  ...
  return (
    <Card 
      style={{ marginBottom: 24, borderRadius: 8, background: isDarkMode ? '#0A0F1C' : '#FAFBFC', border: isDarkMode ? 'none' : 'solid 1px #CBD5E1'}}
        ...
        <Row gutter={[16, 0]}>
          <Col xs={24} sm={12} md={8}>
            <Form.Item name="nf" label="Nota Fiscal (NF)">
                <Input style={{padding:'7px',background: isDarkMode ? '#171C2A' : '#fff', border: isDarkMode ? 'none' : 'solid 1px #CBD5E1'}} placeholder="Buscar por NF" allowClear />
            </Form.Item>
          </Col>
          <Col xs={24} sm={12} md={8}>
